...
exports.selectedDeviceCommand = async (req, res) => {
    try {
        // const host = '.cloudmqtt.com';
        // const port = '15520';
        const { host, port, username, password } = mqttConfig;
        ...
        const selectedDevices = req.body.selectedDevice || []; // Array of selected devices

        const [rows] = await connection.execute('SELECT deviceId FROM devices WHERE afiliateCode = ?', [afiliateCode]);

        // Filter the rows based on selected devices
        const filteredRows = rows.filter(row => selectedDevices.includes(row.deviceId));
        ...
        const subscribePromises = filteredRows.map((row) => {
            return new Promise((resolve) => {
                ...
                mqttClient.subscribe(subscribeTopic, () => {
                    ...
                    mqttClient.publish(commandTopic, JSON.stringify(message), { qos: 0, retain: false }, (error) => {
